...
export function ContactForm({ locale, text, trackingSource = 'contact_form' }: Props) {
  ...
  const gdprCheckboxRef = useRef<HTMLInputElement | null>(null);
  ...
  const gdprCheckboxId = `${fieldIdPrefix}-gdpr`;
  ...
  const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const hasGdprError = !gdprConsent;
    const sourcePath =
      typeof window === 'undefined'
        ? '/'
        : `${window.location.pathname}${window.location.search}${window.location.hash}`;

    if (hasFieldErrors || hasGdprError) {
      setGdprInvalid(hasGdprError);
      setFieldErrors(hasFieldErrors ? nextFieldErrors : {});
      setStatus('error');
      ...
      trackEvent('contact_form_error', {
        ...
        source: trackingSource,
        sourcePath,
        errorCode: hasFieldErrors ? 'validation_invalid' : 'gdpr_consent_missing'
      });
      return;
    }
    ...
    <div className="contact-form-card">
      ...
      <form ref={formRef} className="contact-form" onSubmit={onSubmit} onFocusCapture={trackContactFormStart} noValidate>
        ...
        <div className="contact-gdpr">
          <label className="contact-gdpr-label" htmlFor={gdprCheckboxId}>
            <input
              ref={gdprCheckboxRef}
              id={gdprCheckboxId}
              type="checkbox"
              ...
              onChange={(e) => {
                ...
                if (status === 'error' && gdprInvalid) {
                  setStatus('idle');
                  setFeedback('');
                }
              }}
              aria-invalid={gdprInvalid}
              className="contact-gdpr-checkbox"
            />
            <span className="contact-gdpr-text">{text.gdprConsentLabel}</span>
          </label>
        </div>
